...
class FactoryEnv(DirectRLEnv):
    ...
    def _init_tensors(self):
        ...
        self.keypoints_fingertip = torch.zeros_like(self.keypoints_held, device=self.device)
    ...
    def _compute_intermediate_values(self, dt):
        ...
        if self.cfg_task.name == "nut_bolt_pick":
            ...
            # print("1")
            self.keypoint_dist = torch.norm(self.keypoints_fingertip - self.keypoints_held, p=2, dim=-1).mean(-1)
        else:
    ...
    def _apply_action(self):
        ...
        is_last_20_step = (self.episode_length_buf[0] >= self.max_episode_length - 20)
        # if is_last_20_step.item() and self.cfg_task.name == "nut_bolt_pick":
        #     self.ctrl_target_gripper_dof_pos = 0.0
        #     env_ids = torch.arange(0, 128, device='cuda:0')
        #     self.ctrl_target_joint_pos[:, :7] = 0.0
        ...
        if self.cfg_task.name == "nut_bolt_pick":
            if not is_last_20_step:
                self.ctrl_target_gripper_dof_pos = 0.080
            else:
                self.ctrl_target_joint_pos[:, :7] = torch.tensor([0.0, -57.296, 0.0, -126.051, 0.0, 137.51, 45.837])
                self.ctrl_target_gripper_dof_pos = 0.0
        else:
            self.ctrl_target_gripper_dof_pos = 0.0
        # print(self.ctrl_target_joint_pos[0, :7])
    ...
    def _get_curr_successes(self, success_threshold, check_rot=False):
        ...
        held_cfg = self.cfg_task.held_asset_cfg
        if self.cfg_task.name == "peg_insert" or self.cfg_task.name == "gear_mesh":
            height_threshold = fixed_cfg.height * success_threshold
        elif self.cfg_task.name == "nut_thread":
            height_threshold = fixed_cfg.thread_pitch * success_threshold
        elif self.cfg_task.name == "nut_bolt_pick":
            height_threshold = held_cfg.height * success_threshold
        else:
            raise NotImplementedError("Task not implemented")
        ...
        if self.cfg_task.name == "nut_bolt_pick":
            is_centered = torch.ones_like(curr_successes, dtype=torch.bool)
            is_close_or_below = torch.where(
                z_disp >= height_threshold, torch.ones_like(curr_successes), torch.zeros_like(curr_successes)
            )
    ...
    def _update_rew_buf(self, curr_successes):
        ...
        rew_buf = (
            20* rew_dict["kp_coarse"]
            + 10* rew_dict["kp_baseline"]
            + 5* rew_dict["kp_fine"]
            ...
            + 0*rew_dict["curr_engaged"]
            + 0*rew_dict["curr_successes"]
        )
    ...
    def randomize_initial_state(self, env_ids):
        ...
        if self.cfg_task.name != "nut_bolt_pick":
            ...
            # (2.) Randomize held asset pose (完全遵循固定资产的生成逻辑)
            held_state = self._held_asset.data.default_root_state.clone()[env_ids]
            ...
            held_pos_init_rand = held_pos_init_rand @ torch.diag(held_asset_init_pos_rand)
            held_state[:, 0:3] += held_pos_init_rand + self.scene.env_origins[env_ids]

            # (2.b.) Orientation
            held_orn_init_yaw = np.deg2rad(self.cfg_task.fixed_asset_init_orn_deg)
            held_orn_yaw_range = np.deg2rad(self.cfg_task.fixed_asset_init_orn_range_deg)
            rand_sample = torch.rand((len(env_ids), 3), dtype=torch.float32, device=self.device)
            held_orn_euler = held_orn_init_yaw + held_orn_yaw_range * rand_sample
...
class FactoryEnvPick(DirectRLEnv):
    cfg: FactoryEnvCfg
